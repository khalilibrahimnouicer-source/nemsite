...
const DISPLAY_PHONE = '07 46 38 99 31'
const WA = 'https://wa.me/33746389931'
...
function Logo({ compact = false }) {
  return <div className={`ynr-logo ${compact ? 'compact' : ''}`} aria-label="YNR Luxury — Location de voiture">
    <img src="/logo-ynr-luxury.png" alt="YNR Luxury" />
  </div>
}
...
function Public() {
  ...
  const vehicles = useMemo(() => {
    ...
    if (sort === 'name') return list.sort((a,b) => String(a.name).localeCompare(String(b.name), 'fr', { sensitivity: 'base' }))
    if (sort === 'category') return list.sort((a,b) => String(a.category || '').localeCompare(String(b.category || ''), 'fr', { sensitivity: 'base' }))
  ...
  return <div className="site">
    <header className="public-header">
      ...
      <nav className={`public-nav ${menu ? 'open' : ''}`}>
        ...
        <a href="#why" onClick={() => setMenu(false)}>Notre service</a>
        <a href="#process" onClick={() => setMenu(false)}>Comment ça marche</a>
      ...
      <button type="button" className="mobile-menu" onClick={() => setMenu(v => !v)} aria-expanded={menu} aria-label={menu ? 'Fermer le menu' : 'Ouvrir le menu'}>
    ...
    <main id="top">
      <section className="hero-v2">
        <div className="hero-v2-copy">
          ...
          <div className="hero-contact"><a href={WA} target="_blank" rel="noreferrer">{DISPLAY_PHONE}</a><span>WhatsApp professionnel</span></div>
        </div>
        <div className="hero-v2-media">
          {featured?.photos?.[0] ? <img src={featured.photos[0]} alt={featured.name} /> : <div className="image-placeholder"><div className="hero-placeholder-copy"><span>YNR LUXURY</span><strong>Votre prochaine voiture.</strong></div></div>}
      ...
      <section id="fleet" className="fleet-showcase section-shell">
        <div className="section-heading-row"><div><p className="eyebrow">LA FLOTTE</p><h2>Choisissez votre <em>signature.</em></h2></div><div className="fleet-controls"><label htmlFor="fleet-sort">TRIER PAR</label><select id="fleet-sort" value={sort} onChange={e=>setSort(e.target.value)}><option value="recent">Nouveautés</option><option value="price-asc">Prix : croissant</option><option value="price-desc">Prix : décroissant</option><option value="name">Nom : A → Z</option><option value="category">Catégorie</option></select></div></div>
      ...
      <section id="process" className="process section-shell"><div className="process-head"><p className="eyebrow">COMMENT ÇA MARCHE</p><h2>Simple, du premier clic <em>à la route.</em></h2></div><div className="process-steps"><article><b>01</b><h3>Choisissez</h3><p>Découvrez le véhicule qui correspond à votre besoin.</p></article><article><b>02</b><h3>Demandez</h3><p>Indiquez votre véhicule et vos dates.</p></article><article><b>03</b><h3>Confirmez</h3><p>Échangez directement avec YNR pour finaliser votre demande.</p></article><article><b>04</b><h3>Profitez</h3><p>Votre expérience peut commencer.</p></article></div></section>

      <section id="faq" className="faq-section section-shell"><div className="faq-lead"><p className="eyebrow">QUESTIONS</p><h2>Tout ce qu'il faut <em>savoir.</em></h2></div><div className="faq-list">
...
function RequestForm({ vehicles, onSent }) {
  ...
  return <form className="reservation-form" onSubmit={submit}><label>Véhicule<select name="vehicle" required>{vehicles.map(v => <option key={v.id} value={v.name}>{v.name}</option>)}</select></label><div className="field-grid"><label>Début<input name="start" required type="date"/></label><label>Fin<input name="end" required type="date"/></label></div><label>Nom<input name="name" required autoComplete="name"/></label><label>Téléphone<input name="phone" required placeholder="07 46 38 99 31" autoComplete="tel"/></label><label>E-mail<input name="email" required type="email" autoComplete="email"/></label><input name="website" tabIndex="-1" autoComplete="off" aria-hidden="true" style={{position:'absolute',left:'-10000px',width:1,height:1,opacity:0,pointerEvents:'none'}}/><small style={{display:'block',marginTop:'8px',opacity:.7}}>Les informations envoyées servent uniquement à traiter votre demande de location. Elles sont conservées de manière sécurisée et accessibles uniquement aux personnes habilitées.</small>{error && <p className="form-error">{error}</p>}<button className="button primary" disabled={busy}>{busy ? 'Envoi…' : 'Vérifier la disponibilité'} <ArrowRight size={17}/></button></form>
}
...
function VehicleEditor({ initial, onCancel, onSave, busy }) {
  ...
  async function uploadSelected() {
    if (!files.length) return form.photos
    setUploading(true); setError('')
    try {
      const urls=[]
      const remaining=Math.max(0,8-form.photos.length)
