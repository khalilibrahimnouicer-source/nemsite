...
const PRIVATE_OIDC_TOKEN=String(process.env.VERCEL_OIDC_TOKEN||'').trim()
const privateBlobOptions=()=>({access:'private',useCache:false,storeId:PRIVATE_BLOB_STORE_ID,...(PRIVATE_OIDC_TOKEN?{oidcToken:PRIVATE_OIDC_TOKEN}:{})})
const publicBlobOptions=(extra={})=>({access:'public',token:PUBLIC_BLOB_TOKEN,...extra})
async function read(){
  if(isProd){
    try{
      if(!PRIVATE_BLOB_STORE_ID) throw new Error('PRIVATE_BLOB_READ_WRITE_TOKEN_STORE_ID is missing')
      const result=await get(STORE_BLOB_PATH,privateBlobOptions())
      ...
      if(error?.name==='BlobNotFoundError'){
        ...
        await put(STORE_BLOB_PATH,JSON.stringify(seeded,null,2),{...privateBlobOptions(),allowOverwrite:false,addRandomSuffix:false,contentType:'application/json'})
...
async function write(v){
  ...
  if(isProd){
    try{
      if(!PRIVATE_BLOB_STORE_ID) throw new Error('PRIVATE_BLOB_READ_WRITE_TOKEN_STORE_ID is missing')
      await put(STORE_BLOB_PATH,serialized,{...privateBlobOptions(),allowOverwrite:true,addRandomSuffix:false,contentType:'application/json'})
      return
    }catch(error){
...
app.post('/api/uploads',auth,async(req,res)=>{try{
  ...
  const blob=await put(name,raw,publicBlobOptions({contentType:match[1],addRandomSuffix:false}))
...
app.delete('/api/uploads',auth,async(req,res)=>{try{const url=clean(req.body?.url,200000);if(!url)return res.status(400).json({message:'URL requise'});if(!PUBLIC_BLOB_TOKEN) throw new Error('BLOB_READ_WRITE_TOKEN is missing')
  await del(url,{token:PUBLIC_BLOB_TOKEN});res.sendStatus(204)}catch(error){console.error('[YNR] Blob delete failed',error);res.status(500).json({message:'Impossible de supprimer la photo du stockage'})}})
