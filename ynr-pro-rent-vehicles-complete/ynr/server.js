...
async function read(){
  if(process.env.VERCEL){
    try{
      ...
      if(error?.name!=='BlobNotFoundError') console.warn('[YNR] Persistent store read failed:',error?.message||error)
    }
  }
...
async function write(v){
  ...
  if(process.env.VERCEL){
    try{
      ...
      return
    }catch(error){console.error('[YNR] Persistent Blob store write failed:',error?.message||error)}
  }
