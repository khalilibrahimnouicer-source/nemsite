...
const OWNER_PASSWORD_HASH=String(process.env.OWNER_PASSWORD_HASH||'').trim();const loginAttempts=new Map();
...
const sessionSecret=process.env.SESSION_SECRET||'ynr-development-session-secret';const signSession=exp=>{const value=String(exp),sig=crypto.createHmac('sha256',sessionSecret).update(value).digest('hex');return `${value}.${sig}`};const validSession=t=>{const [exp,sig]=String(t||'').split('.'),expected=crypto.createHmac('sha256',sessionSecret).update(exp||'').digest('hex');return Boolean(exp&&sig&&Number(exp)>Date.now()&&sig.length===expected.length&&crypto.timingSafeEqual(Buffer.from(sig),Buffer.from(expected)))}
const STORE_BLOB_PATH='data/store.json'
const PUBLIC_STORE_BLOB_PATH='data/store.enc.json'
const STORE_KEY=crypto.createHash('sha256').update(String(process.env.SESSION_SECRET||'ynr-development-session-secret')).digest()
function encryptStore(value){
  const iv=crypto.randomBytes(12),cipher=crypto.createCipheriv('aes-256-gcm',STORE_KEY,iv)
...
function decryptStore(text){
  const payload=JSON.parse(text),iv=Buffer.from(payload.iv,'base64'),tag=Buffer.from(payload.tag,'base64'),data=Buffer.from(payload.data,'base64')
  const decipher=crypto.createDecipheriv('aes-256-gcm',STORE_KEY,iv);decipher.setAuthTag(tag)
...
async function read(){
  if(process.env.VERCEL){
    try{
      ...
      if(error?.name!=='BlobNotFoundError') console.warn('[YNR] Private Blob store unavailable, trying encrypted public store:',error?.message||error)
    }
    try{
      const result=await get(PUBLIC_STORE_BLOB_PATH,{access:'public',useCache:false})
      if(result?.stream){const text=await new Response(result.stream).text();const parsed=decryptStore(text);memoryStore=structuredClone(parsed);return parsed}
    }catch(error){
      if(error?.name!=='BlobNotFoundError') console.warn('[YNR] Encrypted Blob store read failed:',error?.message||error)
    }
  }
...
async function write(v){
  ...
  if(process.env.VERCEL){
    try{
      ...
      console.warn('[YNR] Private Blob store unavailable, using encrypted public store:',error?.message||error)
    }
    try{
      await put(PUBLIC_STORE_BLOB_PATH,encryptStore(v),{access:'public',allowOverwrite:true,addRandomSuffix:false,contentType:'application/json',cacheControlMaxAge:0})
      return
    }catch(error){
      console.error('[YNR] Persistent Blob store write failed:',error?.message||error)
      throw new Error('Le stockage des données est indisponible. Vérifiez le Blob Store Vercel du projet.')
    }
  }
  try{await fs.mkdir(dataDir,{recursive:true});const tmp=`${file}.tmp`;await fs.writeFile(tmp,serialized);await fs.rename(tmp,file)}catch(error){if(process.env.VERCEL)console.warn('[YNR] Using memory store on Vercel:',error.code);else throw error}
}
...
const app=express();app.disable('x-powered-by');app.use((_,res,next)=>{res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('Referrer-Policy','strict-origin-when-cross-origin');res.setHeader('Permissions-Policy','camera=(), microphone=(), geolocation=()');next()});app.use(express.json({limit:'25mb'}));app.use(express.static(path.join(root,'dist')))
...
app.post('/api/auth/login',async(req,res)=>{const email=clean(req.body?.email||req.body?.username,180).toLowerCase(),pw=String(req.body?.password||''),ip=String(req.headers['x-forwarded-for']||req.socket.remoteAddress||'').split(',')[0].trim(),key=`${ip}:${email}`,attempt=loginAttempts.get(key)||{count:0,at:Date.now()};if(Date.now()-attempt.at>900000){attempt.count=0;attempt.at=Date.now()}if(attempt.count>=8)return res.status(429).json({message:'Trop de tentatives. Réessayez dans quelques minutes.'});if(!OWNER_PASSWORD_HASH)return res.status(503).json({message:'Authentification administrateur non configurée'});const valid=Boolean(email===OWNER_EMAIL&&await bcrypt.compare(pw,OWNER_PASSWORD_HASH));if(!valid){attempt.count+=1;loginAttempts.set(key,attempt);return res.status(401).json({message:'Identifiants invalides'})}loginAttempts.delete(key);const t=signSession(Date.now()+86400000);const secure=process.env.VERCEL?' Secure;':'';res.setHeader('Set-Cookie',`ynr_session=${t}; HttpOnly; SameSite=Lax;${secure} Path=/; Max-Age=86400`);res.json({ok:true})})
app.get('/api/auth/session',auth,(req,res)=>res.json({ok:true}));app.post('/api/auth/logout',auth,(req,res)=>{const secure=process.env.VERCEL?' Secure;':'';res.setHeader('Set-Cookie',`ynr_session=; HttpOnly; SameSite=Lax;${secure} Path=/; Max-Age=0`);res.status(204).end()})
app.get('/api/admin',auth,async(_,res)=>res.json(await read()))
app.post('/api/requests',async(req,res)=>{const b=req.body||{},name=clean(b.name,120),email=clean(b.email,180).toLowerCase(),vehicle=clean(b.vehicle,120),start=clean(b.start,20),end=clean(b.end,20),emailOk=/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email),startDate=Date.parse(start),endDate=Date.parse(end);if(!name||!emailOk||!vehicle||!start||!end||!Number.isFinite(startDate)||!Number.isFinite(endDate)||endDate<startDate)return res.status(400).json({message:'Veuillez vérifier les champs obligatoires et les dates.'});const s=await read(),item={id:crypto.randomUUID(),name,email,phone:clean(b.phone,40),type:b.type==='professionnel'?'professionnel':'particulier',vehicle,start,end,message:clean(b.message,1500),status:'nouvelle',createdAt:new Date().toISOString()};s.requests.unshift(item);await write(s);res.status(201).json(item)})
